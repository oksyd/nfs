//! Probe one or more NFSv4 paths with Tokio and list directory entries.
//!
//! Usage:
//!   cargo run --example v4_tokio_probe --features tokio -- --check-access --check-file-handle --check-fsinfo --check-public-fh --check-parent --check-named-attrs --write-check --read-bytes 64 127.0.0.1 / /export /export/logs
...
async fn main() -> nfs::Result<()> {
    ...
    for (path_index, path) in config.paths.iter().enumerate() {
        println!();
        if let Err(err) = probe_path(&mut client, path, options, path_index).await {
            failures += 1;
...
impl Config {
    fn from_env() -> nfs::Result<Option<Self>> {
        ...
        let program = args.next().unwrap_or_else(|| "v4_tokio_probe".to_owned());
...
async fn probe_path(
    ...
    let metadata = client.metadata(path).await?;
    print_metadata(path, &metadata);
    if options.check_access {
        print_access(client, path).await?;
    }
    if options.check_secinfo {
        print_secinfo(client, path).await?;
    }
    if options.check_file_handle {
        print_file_handle(client, path).await?;
    }
    if options.check_fsinfo {
        print_fsinfo(client, path).await?;
    }
    if options.check_public_fh {
        print_public_filehandle_metadata(client, path).await?;
    }
    if options.check_parent {
        print_parent_metadata(client, path).await?;
    }
    if options.check_named_attrs {
        print_named_attrs(client, path, options.max_entries).await?;
    }

    if metadata.is_dir()? {
        let entries = client.read_dir_limited(path, options.max_entries).await?;
        ...
        if options.write_check {
            run_write_check(client, path, path_index).await?;
        }
        ...
        let bytes = client.read_range(path, 0, count).await?;
...
async fn print_access(client: &mut Client, path: &str) -> nfs::Result<()> {
    let result = client.access(path, V4_ACCESS_MASK).await?;
    println!(
...
async fn print_secinfo(client: &mut Client, path: &str) -> nfs::Result<()> {
    let flavors = client.secinfo_current(path).await?;
...
async fn print_file_handle(client: &mut Client, path: &str) -> nfs::Result<()> {
    let handle = client.file_handle(path).await?;
    println!("  file-handle: {} bytes", handle.as_bytes().len());
    Ok(())
}

async fn print_fsinfo(client: &mut Client, path: &str) -> nfs::Result<()> {
    let fsinfo = client.fsinfo(path).await?;
    let fsstat = client.fsstat(path).await?;
    let pathconf = client.pathconf(path).await?;
    println!(
...
async fn print_public_filehandle_metadata(client: &mut Client, path: &str) -> nfs::Result<()> {
    if !client.public_exists(path).await? {
        return Err(Error::Protocol(format!(
            "public filehandle path {path} did not resolve"
        )));
    }
    let handle = client.public_file_handle(path).await?;
    let attrs = client.public_getattr(path).await?;
    let access = client.public_access(path, V4_ACCESS_MASK).await?;
    println!(
    ...
    if attrs.is_dir()? {
        let page = client.public_read_dir_page_limited(path, None, 1).await?;
        println!(
...
async fn print_parent_metadata(client: &mut Client, path: &str) -> nfs::Result<()> {
    if !has_parent_path(path) {
        println!("  parent: skipped (root has no parent path)");
        return Ok(());
    }

    let handle = client.parent_file_handle(path).await?;
    let attrs = client.parent_getattr(path).await?;
    let access = client.parent_access(path, V4_ACCESS_MASK).await?;
    println!(
    ...
    if attrs.is_dir()? {
        let page = client.parent_read_dir_page_limited(path, None, 1).await?;
        println!(
...
async fn print_named_attrs(client: &mut Client, path: &str, max_entries: usize) -> nfs::Result<()> {
    let entries = client.read_named_attrs_limited(path, max_entries).await?;
...
async fn run_write_check(client: &mut Client, dir: &str, path_index: usize) -> nfs::Result<()> {
    ...
    let result = async {
        client.write_atomic(&file, WRITE_CHECK_CONTENT).await?;
    ...
    let cleanup = client.remove_if_exists(&file).await;
