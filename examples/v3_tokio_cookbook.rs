//! NFSv3 Tokio cookbook.
//!
//! Run against a writable export:
//!
//! ```text
//! cargo run --example v3_tokio_cookbook --features tokio -- 127.0.0.1:/export /writable-dir
//! ```
//!
...
const OBJECT: &[u8] = b"async nfs-rs v3 cookbook object\nline two\nline three\n";
const STREAMED: &[u8] = b"streamed through async write_from_reader\n";
...
async fn main() -> nfs::Result<()> {
    ...
    let work_dir = remote_path(
        &config.remote_dir,
        &format!("nfs-rs-v3-tokio-cookbook-{}", std::process::id()),
    );
    ...
    let unmount_result = client.unmount().await;
    finish(flow_result, cleanup_result, unmount_result)
}

async fn connect(target: &str) -> nfs::Result<Client> {
...
async fn run_filesystem_flow(client: &mut Client, work_dir: &str) -> nfs::Result<()> {
    ...
    client
        .write_atomic_with_mode_and_stability(&object, OBJECT, 0o640, StableHow::FileSync)
        .await?;
    print_v3_metadata(
    ...
    let uploaded_len = client
        .write_atomic_from_reader_with_mode_and_stability(
            &streamed,
            &mut upload,
            0o644,
            StableHow::FileSync,
    ...
    client
        .append_with_stability(&streamed, b"appended\n", StableHow::FileSync)
        .await?;
    client
        .write_at_with_stability(&streamed, 0, b"STREAMED", StableHow::FileSync)
        .await?;
    client.truncate(&streamed, 24).await?;
    let commit = client.commit(&streamed, 0, 0).await?;
    println!("commit verifier: {:02x?}", commit.verifier);

    client.create_new(&created, 0o600).await?;
    client
        .write_at_with_stability(
            &created,
            0,
    ...
    let copied_len = client
        .copy_atomic_with_stability(&streamed, &copied, StableHow::FileSync)
        .await?;
