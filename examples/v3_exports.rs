//! List exports advertised by an NFSv3 server's MOUNT service.
//!
//! Usage:
//!   cargo run --example v3_exports -- --show-mounts 127.0.0.1
...
fn main() -> nfs::Result<()> {
    ...
    let exports = match config.mount_port {
        Some(port) => list_exports_with_mount_port(&config.host, port, config.timeout)?,
        None => list_exports_with_timeout(&config.host, config.timeout)?,
    };
    ...
    if config.show_mounts {
        let mounts = match config.mount_port {
            Some(port) => list_mounts_with_mount_port(&config.host, port, config.timeout)?,
            None => list_mounts_with_timeout(&config.host, config.timeout)?,
        };
        println!();
...
impl Config {
    fn from_env() -> nfs::Result<Option<Self>> {
        ...
        let program = args.next().unwrap_or_else(|| "v3_exports".to_owned());
        let mut host = None;
