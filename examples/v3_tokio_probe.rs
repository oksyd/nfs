//! Probe one or more paths inside an NFSv3 export with Tokio.
//!
//! Usage:
//!   cargo run --example v3_tokio_probe --features tokio -- --check-access --check-file-handle --check-fsinfo --check-parent --write-check --read-bytes 64 127.0.0.1:/export / /logs /data

use std::time::Duration;

use nfs::v3::tokio::{Client, ClientBuilder};
use nfs::v3::{
...
async fn main() -> nfs::Result<()> {
    ...
    println!("connected to async NFSv3 target {}", config.target);
    println!(
    ...
    if let Err(err) = client.unmount().await {
...
impl Config {
    fn from_env() -> nfs::Result<Option<Self>> {
        ...
        let program = args.next().unwrap_or_else(|| "v3_tokio_probe".to_owned());
...
async fn print_access(client: &mut Client, path: &str) -> nfs::Result<()> {
    let result = client.access(path, V3_ACCESS_MASK).await?;
    println!(
...
async fn print_fsinfo(client: &mut Client, path: &str) -> nfs::Result<()> {
    let info = client.path_fsinfo(path).await?;
    println!(
