...
async fn run_filesystem_flow(client: &mut Client, work_dir: &str) -> nfs::Result<()> {
    ...
    client.create_new_with_mode(&offloaded, 0o644).await?;
    if let Some(response) = optional(
        "copy_range_offload",
        client
            .copy_range_offload(&streamed, &offloaded, 0, 0, copied_len)
            .await,
    )
    ...
    demonstrate_v4_locking(client, &object).await?;
...
async fn demonstrate_v4_locking(client: &mut Client, object: &str) -> nfs::Result<()> {
    if let Some(conflict) = optional(
        "test_lock",
        client.test_lock(object, LockType::Read, 0, 16).await,
    ) {
        println!("read lock conflict: {}", conflict.is_some());
    }

    if let Some(lock) = optional("read_lock", client.read_lock(object, 0, 16).await) {
        println!(
        ...
        if let Some(status) = optional("test_stateid", client.test_stateid(lock.stateid()).await) {
