...
pub struct Client {
    ...
    lease_renewal: Option<super::lease::TokioLease>,
...
impl Client {
    ...
    async fn recover_session(&mut self) -> Result<()> {
        ...
        let _ = self.stop_lease_renewal().await;
        let mut retry = 0;
        let mut rebuilt = loop {
            match Self::connect_session(self.builder.clone(), false).await {
        ...
        for (index, state) in states.iter_mut().enumerate() {
            ...
            let recovered = if state.client_id == rebuilt.client_id {
                rebuilt
                    .check_recovered_lock(state)
                    .await
                    .map(|()| state.clone())
            } else {
                rebuilt.reclaim_lock(state).await
            };
            match recovered {
        ...
        let response = rebuilt
            .recovery_compound(vec![Operation::ReclaimComplete { one_fs: false }])
            .await?;
        ensure_reclaim_complete(&response)?;
        if self.root_fsinfo.is_some() {
            rebuilt.refresh_recovered_fsinfo().await?;
        }
        rebuilt.start_lease_renewal().await?;
        let old = std::mem::replace(self, rebuilt);
        let _ = old.shutdown().await;
        self.locks.ensure_valid()
    }

    async fn refresh_recovered_fsinfo(&mut self) -> Result<()> {
        let response = self
            .recovery_compound(vec![
                Operation::PutRootFh,
        ...
        let attrs = if attrmask.is_empty() {
            ...
            let response = self
                .recovery_compound(vec![Operation::PutRootFh, Operation::GetAttr(attrmask)])
                .await?;
            response.ensure_ok()?;
            response_getattr(&response)?
        };
    ...
    async fn check_revoked_locks(&mut self) -> Result<()> {
        for (index, mut lock) in self.locks.snapshot().into_iter().enumerate() {
            ...
            let response = self
                .recovery_compound(vec![Operation::TestStateIds(ids.clone())])
                .await?;
            response.ensure_ok()?;
            ...
            for (id, status) in ids.into_iter().zip(statuses) {
                if !status.is_ok() {
                    ...
                    let _ = self
                        .recovery_compound(vec![Operation::FreeStateId(id)])
                        .await;
                }
            }
        }
    ...
    async fn check_recovered_lock(&mut self, lock: &LockState) -> Result<()> {
        let response = self
            .recovery_compound(vec![Operation::TestStateIds(vec![
                lock.open_stateid,
                lock.lock_stateid,
            ])])
            .await?;
    ...
    async fn reclaim_lock(&mut self, lock: &LockState) -> Result<LockState> {
        let response = self
            .recovery_compound(vec![
                Operation::PutFh(lock.handle.clone()),
                Operation::Open(OpenArgs {
                    seqid: self.current_open_seqid(),
                    share_access: lock_share_access(lock.lock_type)
    ...
    async fn recovery_compound(&mut self, operations: Vec<Operation>) -> Result<CompoundResponse> {
        ...
        loop {
            ...
            let response = self
                .raw_compound("nfs-rs-recovery", self.minor_version, compound)
                .await?;
    ...
    pub async fn unlock(&mut self, lock: ByteRangeLock) -> Result<()> {
        ...
        let result = match self.unlock_opened(&state).await {
            Ok(stateid) => match self.free_stateid(stateid).await {
                Ok(()) => self.release_lock_owner(state.owner).await.map(|_| ()),
                Err(err) => Err(err),
            },
            Err(err) => Err(err),
        };
    ...
    async fn stop_lease_renewal(&mut self) -> Result<()> {
        if let Some(lease) = self.lease_renewal.take() {
            let session_id = lease.stop().await;
            let response = self
                .raw_compound(
    ...
    async fn start_lease_renewal(&mut self) -> Result<()> {
        ...
        let mut keeper = Self::connect_session(builder, false).await?;
        if keeper.client_id != self.client_id {
            let _ = keeper.shutdown().await;
            return Err(Error::nfsv4("EXCHANGE_ID", Status::StaleClientId));
        }
        let session_id = keeper.session_id;
        let health = std::sync::Arc::new(super::lease::LeaseHealth::default());
        let worker_health = health.clone();
        let task = ::tokio::spawn(async move {
            loop {
                ::tokio::time::sleep(interval).await;
                let healthy = keeper
                    .recovery_compound(Vec::new())
                    .await
                    .is_ok_and(|response| {
        ...
        self.lease_renewal = Some(super::lease::TokioLease {
            health,
            task,
            session_id,
        });
        ...
        loop {
                ...
                .await
            {
                ...
                Err(err) if err.is_transport_failure() && !recovered_session => {
                    self.recovery_pending = true;
                    if let Err(recovery) = self.recover_session().await {
            ...
            if crate::v4::client::response_revoked_lock_status(&response).is_some() {
                self.check_revoked_locks().await?;
                if !crate::v4::client::operations_release_state(&operations) {
                    self.locks.ensure_valid()?;
                }
            }
    ...
    async fn connect_with_builder(builder: ClientBuilder) -> Result<Self> {
        let mut client = Self::connect_session(builder, true).await?;
        ...
        if let Err(err) = client.start_lease_renewal().await {
            return Err(cleanup_error(
                err,
    ...
    async fn connect_session(builder: ClientBuilder, complete_reclaim: bool) -> Result<Self> {
    ...
    async fn connect_session_minor(
        builder: ClientBuilder,
        minor_version: u32,
...
impl Client {
    ...
    async fn unlock_opened(&mut self, lock: &LockState) -> Result<StateId> {
        let mut retry = 0;
...
mod recovery_regressions {
    ...
    async fn restart_reclaims_open_and_lock_before_reclaim_complete() {
        let server = restart_script(None);
        let mut client = Client::connect_session(builder(server.addr), true)
            .await
            .unwrap();
        let lock = client.locks.register(lock_state());
        client.recover_session().await.unwrap();
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn expired_grace_reports_lost_lock_and_blocks_further_io() {
        let server = restart_script(Some(Status::NoGrace));
        let mut client = Client::connect_session(builder(server.addr), true)
            .await
            .unwrap();
        let lock = client.locks.register(lock_state());
        assert!(client.recover_session().await.unwrap_err().is_lost_state());
        assert!(lock.is_lost());
        assert!(client.renew().await.unwrap_err().is_lost_state());
        assert!(client.unlock(lock).await.unwrap_err().is_lost_state());
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn read_only_rpc_recovers_after_disconnect() {
        let (server, ops) = disconnect_script(false);
        let mut client = Client::connect_session(builder(server.addr), true)
            .await
            .unwrap();
        client.compound(ops).await.unwrap();
        drop(client);
        server.finish();
    }

    #[tokio::test(flavor = "current_thread")]
    async fn mutation_with_lost_reply_is_not_replayed() {
        let (server, ops) = disconnect_script(true);
        let mut client = Client::connect_session(builder(server.addr), true)
            .await
            .unwrap();
        let err = client.compound(ops).await.unwrap_err();
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn session_reconnect_preserves_existing_locks() {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn background_renewal_uses_separate_session_and_shuts_it_down() {
        ...
        client.start_lease_renewal().await.unwrap();
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn interrupted_recovery_keeps_successfully_reclaimed_state() {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn partial_revocation_checks_actual_state_and_acknowledges_only_lost_locks() {
        for lost in [false, true] {
            ...
            let result = client.unlock(lock).await;
            if lost {
                assert!(result.unwrap_err().is_lost_state());
            } else {
                result.unwrap();
